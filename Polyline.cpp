...
struct Point
{
    ...
    friend T dist(const Point<T>& a, const Point<T>& b)
    {
        T x = (a.x - b.x);
        T y = (a.y - b.y);
        return x * x + y * y;
...
public:
    ...
    Polyline(const T& m1, const T& m2, int size)
    {
        ...
        {
            std::uniform_real_distribution<> distrib(m1, m2);
            for (int i = 0; i < m_size; i++) {
                T randomX = distrib(gen);
                T randomY = distrib(gen);
                points[i] = Point<T>(randomX, randomY);
    ...
    friend std::ostream& operator << (std::ostream& out, const Polyline<T>& pol)
    {
        out << "[ ";
